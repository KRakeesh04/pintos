...
#ifdef USERPROG
/* Structure to track child process state.
   This persists even after the child thread exits. */
struct child_status
{
   tid_t tid;                  /* Child's thread ID. */
   int exit_status;            /* Child's exit status. */
   bool exited;                /* Has child exited? */
   bool waited_on;             /* Has parent waited on this child? */
   bool load_success;          /* Did load succeed? */
...
struct thread
{
   ...
   /* Process control block info. */
   struct thread *parent;          /* Parent process. */
   struct list children;           /* List of child_status structures. */
   struct child_status *my_status; /* My status in parent's children list. */
